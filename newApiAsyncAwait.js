async function trackGetRequest(url) {
  let fetchTracks = await fetch(url);
  let myResponse = await fetchTracks.json();
  trackResponse(myResponse);
}
...
async function albumGetRequest(url) {
  try {
    let fetchAlbum = await fetch(url);
    let myResponse2 = await fetchAlbum.json();
    albumResponse(myResponse2);
  } catch (err) {
    console.log(err);
  }
}
